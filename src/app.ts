    ...
    const drawFrame = (frameIndex: number, renderScale: number) => {
      ...
      for (const part of getCompositePartsInDrawOrder()) {
        ...
        if (animationMode === 'part-based-small-walk') {
          ...
          if (role === 'torso') { driftX = (-compress * 1.2 + pull * 2.4 + lurch * 5.8 - recoil * 2.2 - settle) * lurchAmt; bobY = (compress * 2.8 - lurch * 1.9 + recoil * 1.6 + settle * 0.9) * 1.1; sx = 1 + (-compress * 0.055 + lurch * 0.045) * squash; sy = 1 + (compress * 0.065 - lurch * 0.04) * squash; rot = (-compress * 1.2 + lurch * 1.1 - recoil * 0.6) * squash; }
          ...
          else if (role === 'front_leg' || role === 'rear_leg') { bobY = compress * 0.3; rot = (-pull * 1.2 + recoil * 1.4) * 0.6; }
