...
export function initApp(root: HTMLDivElement) {
  ...
  const defaultAttackSettings: AttackSettings = { attackIntensity: 1, attackReach: 1, torsoLean: 1, armSwing: 1, recoilAmount: 1 };
  ...
  const generateStripAndPreview = () => {
    if (!image || !state.analysis) return;
    if ((animationMode === 'part-based-idle' || animationMode === 'part-based-small-walk' || animationMode === 'part-based-attack') && !extractedPartLayers.size) { setStatus('Build Part Layers first.', true); return; }
    ...
    const drawFrame = (frameIndex: number, renderScale: number) => {
      ...
      for (const part of parts.slice()) {
        ...
        if (animationMode === 'part-based-small-walk') {
          ...
          if (role === 'torso') { driftX = (strike * 7 * reach - recoil * 4.5 * recoilAmt) * intensityAttack; bobY = -strike * 1.2 * intensityAttack; rot = (strike * 6 * lean - recoil * 5 * recoilAmt) * intensityAttack; }
