...
export function initApp(root: HTMLDivElement) {
  ...
  const defaultSeamRepairSettings: SeamRepairSettings = { enabled: true, edgeBleedPx: 2, edgeFeatherPx: 1, jointOverlapPx: 5, gapFillEnabled: true, seamBlendStrength: 50 };
  ...
  const buildPartLayers = () => {
    ...
    for (const part of parts) {
      ...
      const prepared = seamRepairSettings.enabled ? applySimpleBleed(layer, seamRepairSettings.edgeBleedPx, seamRepairSettings.edgeFeatherPx) : layer;
      extractedPartLayers.set(part.name, prepared);
    }
