...
const syncShellModeControls = () => {
  ...
  const hasJawRig = () => rawExtractedPartLayers.has('lower_jaw') && pivots.has('lower_jaw');
  const getJawOverlayRotationDeg = (frameIndex: number): number => {
    if (!jawAnimationEnabled || !hasJawRig()) return 0;
  ...
  const syncJawUi = () => {
    const available = hasJawRig();
    q<HTMLDivElement>('jawMotionSection').hidden = false;
    q<HTMLParagraphElement>('jawMotionHint').hidden = available;
    q<HTMLDivElement>('jawMotionControls').hidden = !available;
    ...
    q<HTMLDivElement>('jawModeRow').hidden = !jawAnimationEnabled;
    q<HTMLDivElement>('jawSettingsGrid').hidden = !jawAnimationEnabled;
