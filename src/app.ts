...
export function initApp(root: HTMLDivElement) {
  ...
  root.innerHTML = `<div class="shell"><header class="topBar panel"><div><h1>Sprite Rig Lab</h1><p id="status" class="status">Waiting for PNG upload.</p><p id="imageMeta" class="status">No source image loaded.</p></div><div class="projectButtons"><label class="fileLabel">Upload PNG<input id="file" type="file" accept="image/png" /></label><label class="fileLabel">Load Project JSON<input id="loadProject" type="file" accept="application/json" /></label><button id="saveProject" disabled>Save Project JSON</button></div></header>
  ...
  <section class="panel modeControls"><div class="partChips" id="partChips"></div><div id="partInfo" class="partInfo"></div>
  <div id="maskControls"><div class="segmented toolModes"><button id="brushAddMode" type="button">Brush Add</button><button id="brushEraseMode" type="button">Brush Erase</button><button id="lassoAddMode" type="button">Lasso Add</button><button id="lassoEraseMode" type="button">Lasso Erase</button></div><div class="row"><button id="undoMaskAction" type="button" disabled>Undo</button><button id="cancelLasso" type="button" disabled>Cancel Lasso</button></div><div class="compactSlider"><label for="brushSize">Brush <span id="brushSizeValue">24</span></label><input id="brushSize" type="range" min="1" max="256" value="24" /></div><div class="compactSlider"><label for="overlayOpacity">Overlay <span id="overlayOpacityValue">45%</span></label><input id="overlayOpacity" type="range" min="0.05" max="1" step="0.05" value="0.45" /></div></div>
  ...
  const status = q<HTMLParagraphElement>('status'); const renderReport = q<HTMLPreElement>('renderReport'); const sourceQuality = q<HTMLPreElement>('sourceQuality'); const imageMeta = q<HTMLParagraphElement>('imageMeta');
