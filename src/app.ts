...
export function initApp(root: HTMLDivElement) {
  ...
  const syncIdleReadout = () => {
    ...
    q<HTMLDivElement>('idleControlGrid').hidden = animationMode === 'part-based-small-walk';
  ...
  q<HTMLButtonElement>('partWalkMode').addEventListener('click', () => { animationMode = 'part-based-small-walk'; markStale(); syncIdleReadout(); });
  ...
  ['walkIntensity','strideWidth','legCrossing','hipSway','armSwing','footLockStrength'].forEach((key)=> q<HTMLInputElement>(key).addEventListener('input', (e)=> { (walkSettings as any)[key] = Number((e.target as HTMLInputElement).value); markStale(); syncIdleReadout(); }));
