...
export function initApp(root: HTMLDivElement) {
  ...
  const defaultQuadrupedWalkSettings: QuadrupedWalkSettings = { stepLength: 1, legLift: 1, bodyBob: 1, headBob: 1, gaitIntensity: 1 };
...
const seamJointPairs = new Set(['torso:front_arm','torso:rear_arm','torso:front_leg','torso:rear_leg','torso:head']);
  ...
  const generateStripAndPreview = () => {
    if (!image || !state.analysis) return;
    if ((animationMode === 'part-based-idle' || animationMode === 'part-based-small-walk' || animationMode === 'part-based-attack' || animationMode === 'part-based-globular-crawl') && !rawExtractedPartLayers.size) { setStatus('Build Part Layers first.', true); return; }
...
const syncShellModeControls = () => {
  ...
  q<HTMLButtonElement>('resetQuadrupedSettings').addEventListener('click', () => { quadrupedWalkSettings = { ...defaultQuadrupedWalkSettings }; ['quadStepLength','quadLegLift','quadBodyBob','quadHeadBob','quadGaitIntensity'].forEach((key)=> { q<HTMLInputElement>(key).value = '1'; }); markStale(); syncIdleReadout(); });
