...
export function initApp(root: HTMLDivElement) {
  ...
  root.innerHTML = `<div class="shell"><header class="topBar panel"><div class="brandBlock"><h1>SPRITE RIG LAB</h1><p id="status" class="status">Waiting for PNG upload.</p></div><div class="projectButtons"><label class="fileLabel">Upload PNG<input id="file" type="file" accept="image/png" /></label><label class="fileLabel">Load Project<input id="loadProject" type="file" accept="application/json" /></label><button id="saveProject" disabled>Save Project</button></div></header>
  <section class="panel statusRow"><span id="fileStat">File: none</span><span id="dimensionsStat">Dimensions: —</span><span id="partsStat">Parts: 0</span><span id="modeStat">Mode: Mask</span><span id="zoomStat">Zoom: 100%</span><span id="savedStat" class="savedBadge">Unsaved</span></section>
