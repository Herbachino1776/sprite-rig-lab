...
export function exportFpvMetadata(state: FpvState, frameOffsets: FpvFrameOffsets[], qa: FpvQaReport): FpvMetadata {
  ...
  return {
    ...
    animationSettings: {
      idle: { ...state.animationSettings.idle },
      punchClaw: { ...state.animationSettings.punchClaw },
    },
    previewSettings: { selectedFrame: state.selectedFrame, previewPlaying: state.previewPlaying, previewFps: state.previewFps },
    layers: state.layers.map((layer, layerOrder) => ({
      ...
      layerOrder,
      visible: layer.baseTransform.visible,
      opacity: layer.baseTransform.opacity,
